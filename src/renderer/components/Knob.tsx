...
export function Knob({
  ...
  const hardwareName = `K${index + 1}`;

  // Convert level to percentage for meter display (0-100)
  // Using RMS for smoother display, clamped to 0-1 range
  const levelPercent = level ? Math.min(100, Math.round(level.rms * 100 * 3)) : 0; // Scale up RMS for visibility
  ...
  const handleSave = () => {
    setIsEditing(false);
    const effectiveChannelId = channelId || `k${index + 1}`;
